...
public class DisplayActivity extends AppCompatActivity implements BundleListener, ImageListener, AdapterView.OnItemClickListener {
    ...
    BroadcastReceiver receiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, Intent intent) {
            if (intent.getAction() != null && intent.getAction().equals(Const.BROADCAST_PACKAGES_RESULT)) {
                if (mPackageManager.getPackages().size() == 1) {
                    setPackageSelected(mPackageManager.getPackages().get(0));
                }
            }
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPackageManager = PackageManager.getInstance();
    ...
    public void onSelectPackage(View v) {
        final List<Package> packages = mPackageManager.getPackages();
    ...
    private boolean updateTrailInfo() {
        if (!TextUtils.isEmpty(selectedLanguage) && selectedEnvironment != null) {
            GetPackageDetails getPackageDetails = new GetPackageDetails(this, selectedPackage, this);
            getPackageDetails.query(selectedEnvironment, selectedLanguage);
            return true;
        }
        return false;
    }
