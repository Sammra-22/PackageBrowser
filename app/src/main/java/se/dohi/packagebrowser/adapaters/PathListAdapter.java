    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(FileUtils.imageExists(getContext(), path.getName())) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                holder.pathImageView.setBackground(Drawable.createFromPath(FileUtils.getImageFile(getContext(), path.getName()).getAbsolutePath()));
            }else
                holder.pathImageView.setBackgroundDrawable(Drawable.createFromPath(FileUtils.getImageFile(getContext(), path.getName()).getAbsolutePath()));
        }
        holder.pathNameView.setText(path.getName());
        holder.pathInfoView.setText(path.getInfo());
        holder.pathLengthView.setText("Length: "+path.getLength()+" Km");
        holder.pathTimeView.setText("Duration: "+path.getTime()+" H");
